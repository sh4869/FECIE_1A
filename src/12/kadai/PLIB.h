...
void binary_tree(BSTnode **head,int *data,int data_number){
  BSTnode *v,*up,*nr;
  int i,x;
  printf("(** Binary Tree using data[%d] **)\n",data_number);
  if(data_number<1){
    *head = NULL;
    printf("No Binary Tree (end)\n");
    return;
  }
  *head = v = node(data[0]);
  for(i=1;i<data_number;i++){
    ...
    while(v != NULL){
      ...
      if(x < v->key){
        ...
        printf("data[%d]=%d(binary_tree,The same data already exists (end)\n)",i,data[i]);
        return;
      }
    }
    nr = node(x);
